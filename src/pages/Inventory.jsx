...
function Inventory() {
  ...
  useEffect(() => {
    ...
    // Listen for custom event when inventory data changes
  ...
  const loadInventoryData = () => {
    ...
    const items = products.map(product => ({
      ...
      currentStock: product.stock !== undefined ? product.stock : '-',
      minStock: product.lowStockThreshold || 0,
      supplier: product.vendor,
      isLowStock: product.stock !== undefined && product.lowStockThreshold !== undefined && 
  ...
  const handleUpdateConfirm = (reasonData) => {
    ...
    if (remainingProductIds.length > 0) {
      ...
      // Dispatch custom event with detail to notify other components about inventory changes
      // Using CustomEvent instead of Event to pass data
      const inventoryEvent = new CustomEvent('inventoryDataChanged', {
        detail: { timestamp: Date.now() }
      });
      window.dispatchEvent(inventoryEvent);
      
      // Force a direct update to the dashboard by calling the global function if it exists
    ...
    <Container>
      <Header>
        ...
        <div className="action-buttons">
          <ActionButton className="success" onClick={() => setShowImportExportModal(true)}>
            <FiDownload />
            Import/Export
          </ActionButton>
