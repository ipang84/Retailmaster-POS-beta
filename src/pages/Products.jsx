...
import { FiArrowLeft, FiPlus, FiEdit2, FiTrash2, FiFilter, FiSearch, FiMoreVertical, FiDownload, FiUpload, FiPrinter } from 'react-icons/fi';
...
function Products() {
  ...
  useEffect(() => {
    ...
    // Listen for custom event when product data changes
    const handleProductDataChanged = () => {
      loadProducts();
    };
    
    window.addEventListener('productDataChanged', handleProductDataChanged);
    
    return () => {
      window.removeEventListener('productDataChanged', handleProductDataChanged);
    };
  }, []);
  ...
  // Format the status display text - Fixed to handle undefined status
  const formatStatusText = (status) => {
    if (!status) return 'Unknown';
    if (status === 'partial-<boltAction type="file" filePath="src/pages/Products.jsx">refunded') return 'Partially Refunded';
    ...
    <Container>
      <Header>
        ...
        <div className="right-section">
          <ActionButton className="secondary" onClick={() => setShowImportExportModal(true)}>
            <FiDownload />
            Import/Export
          </ActionButton>
          ...
          <tbody>
            {products.map((product, index) => (
              <tr key={product.id}>
                ...
                <td>
                  <div className={`status-badge ${product.status || 'unknown'}`}>
