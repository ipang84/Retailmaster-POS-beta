...
import { endRegisterSession, calculateTotalCash } from '../services/registerService';
...
function EndRegisterSessionModal({ isOpen, onClose, onSessionEnd, currentSession }) {
  // Denominations
  ...
  useEffect(() => {
    ...
    // Calculate difference if we have a current session
    if (currentSession) {
        ...
        <ModalContainer>
          <SuccessMessage>
            ...
            <div className="message">
              Your register session has been ended successfully with a final cash count of ${total.toFixed(2)}.
            </div>
          </SuccessMessage>
        </ModalContainer>
      </ModalOverlay>
  ...
  return (
    <ModalOverlay>
      <ModalContainer>
        ...
        {currentSession && (
          <TotalSection>
            <TotalLabel>Difference from Expected</TotalLabel>
            <TotalAmount style={{ color: difference >= 0 ? 'var(--success-color)' : 'var(--danger-color)' }}>
